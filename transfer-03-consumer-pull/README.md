...
```bash
./gradlew transfer-03-consumer-pull:provider-proxy-data-plane:build
```
...
```bash
java -Dedc.fs.config=transfer-03-consumer-pull/resources/configuration/provider.properties \
    -jar transfer-03-consumer-pull/provider-proxy-data-plane/build/libs/connector.jar
```
...
```bash
java -Dedc.fs.config=transfer-00-prerequisites/resources/configuration/consumer-configuration.properties \
  -jar transfer-00-prerequisites/connector/build/libs/connector.jar
```
...
curl -X POST "http://localhost:29193/management/v3/transferprocesses" \
  ...
  -d @transfer-03-consumer-pull/resources/start-transfer.json \
  -s | jq
```
