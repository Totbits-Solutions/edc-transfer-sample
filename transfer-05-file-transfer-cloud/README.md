...
```bash
docker compose -f transfer-05-file-transfer-cloud/resources/docker-compose.yaml up -d
```
...
```bash
az storage blob upload -f ./transfer-05-file-transfer-cloud/resources/test-document.txt --container-name src-container --name test-document.txt --connection-string $conn_str
```
...
```bash
./gradlew clean build
java -Dedc.fs.config=transfer-05-file-transfer-cloud/cloud-transfer-provider/config.properties -jar transfer-05-file-transfer-cloud/cloud-transfer-provider/build/libs/provider.jar
# in another terminal window:
java -Dedc.fs.config=transfer-05-file-transfer-cloud/cloud-transfer-consumer/config.properties -jar transfer-05-file-transfer-cloud/cloud-transfer-consumer/build/libs/consumer.jar
```
...
curl -X POST "http://localhost:29193/management/v3/catalog/request" \
    ...
    -d @transfer-05-file-transfer-cloud/resources/fetch-catalog.json -s | jq
```
...
```bash
curl -d @transfer-05-file-transfer-cloud/resources/negotiate-contract.json \
...
curl -X POST "http://localhost:29193/management/v3/transferprocesses" \
  -H 'X-Api-Key: password' -H "Content-Type: application/json" \
  -d @transfer-05-file-transfer-cloud/resources/start-transfer.json \
  -s | jq
```
...
```bash
docker compose -f transfer-05-file-transfer-cloud/resources/docker-compose.yaml down
```
